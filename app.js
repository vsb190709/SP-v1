...
function renderAll(){const base=totals(), pr=periodRows(), t={inc:pr.filter(r=>r.type==="income").reduce((a,r)=>a+r.amount,0),exp:pr.filter(r=>r.type==="expense").reduce((a,r)=>a+r.amount,0),pInc:pr.filter(r=>r.type==="income"&&r.status==="Pending").reduce((a,r)=>a+r.amount,0),pExp:pr.filter(r=>r.type==="expense"&&r.status==="Pending").reduce((a,r)=>a+r.amount,0),cash:pr.filter(r=>r.mode==="Cash"&&((r.type==="income"&&r.status==="Received")||(r.type==="expense"&&r.status==="Paid"))).reduce((a,r)=>a+(r.type==="income"?r.amount:-r.amount),0),upi:pr.filter(r=>r.mode==="UPI"&&((r.type==="income"&&r.status==="Received")||(r.type==="expense"&&r.status==="Paid"))).reduce((a,r)=>a+(r.type==="income"?r.amount:-r.amount),0)};t.balance=t.inc-t.exp-t.pInc+t.pExp;t.expected=t.inc-t.exp;$("#mandalTitle").textContent=state.settings.mandalName;$("#balance").textContent=money(t.balance);$("#expected").textContent=`Expected ${money(t.expected)}`;$("#incomeTotal").textContent=money(t.inc);$("#incomePending").textContent=`${money(t.pInc)} pending`;$("#expenseTotal").textContent=money(t.exp);$("#expensePending").textContent=`${money(t.pExp)} pending`;const td=state.records.filter(r=>r.date===today());$("#todayIncome").textContent=money(td.filter(r=>r.type==="income"&&r.status==="Received").reduce((a,r)=>a+r.amount,0));$("#todayExpense").textContent=`Expenses ${money(td.filter(r=>r.type==="expense"&&r.status==="Paid").reduce((a,r)=>a+r.amount,0))}`;$("#cashTotal").textContent=money(t.cash);$("#upiTotal").textContent=`UPI ${money(t.upi)}`;$("#rReceived").textContent=money(t.inc-t.pInc);$("#rPaid").textContent=money(t.exp-t.pExp);$("#rBalance").textContent=money(t.balance);$("#rPending").textContent=money(t.pInc-t.pExp);renderRecent();renderReports();renderDashboardExtras();if($("#incomeForm")){$("#incomeForm").date.value=today();$("#expenseForm").date.value=today()}}
...
function periodRows(){
  const p=$("#dashPeriod")?.value||"today", now=new Date(), d=new Date(now.getFullYear(),now.getMonth(),now.getDate());
  if(p==="all") return state.records;
  if(p==="today"){const s=today();return state.records.filter(r=>r.date===s)}
  if(p==="week"){const day=(d.getDay()+6)%7;const s=new Date(d);s.setDate(d.getDate()-day);const e=new Date(s);e.setDate(s.getDate()+6);return state.records.filter(r=>r.date>=s.toISOString().slice(0,10)&&r.date<=e.toISOString().slice(0,10))}
  if(p==="month"){const s=new Date(d.getFullYear(),d.getMonth(),1),e=new Date(d.getFullYear(),d.getMonth()+1,0);return state.records.filter(r=>r.date>=s.toISOString().slice(0,10)&&r.date<=e.toISOString().slice(0,10))}
  const y=state.settings.year||now.getFullYear(); return state.records.filter(r=>String(r.date).startsWith(String(y)));
}
