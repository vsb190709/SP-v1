...
function renderAll(){
  ...
  modes.forEach(mode=>{
    ...
    const me=pr.filter(r=>r.type==="expense"&&r.mode===mode&&r.status==="Paid").reduce((a,r)=>a+r.amount,0);
    const net=mi-me; modeNet+=net;
    const el=$("#"+mode.toLowerCase()+"Total"); if(el) el.textContent=money(net);
  });
  $("#modeNetTotal").textContent=money(modeNet);
